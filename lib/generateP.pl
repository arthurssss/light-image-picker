...
if (length($f) == 0) {
  $f = 'build/generated/source/r/debug/net/neevek/android/lib/lightimagepicker/R.java'
  #print("usage: ./generateP <filename>\n");
  #exit;
}
open my $fh, $f or die "failed to open file: " . $f;

my $out_file = "src/main/java/net/neevek/android/lib/lightimagepicker/P.java";
open my $out_handle, '>', $out_file or die "failed to open output file: " . $out_file;

print $out_handle "package net.neevek.android.lib.lightimagepicker;\n\n\n";
...
while(<$fh>) {
  ...
  if ($should_print && /light_image_picker_/) {
    s/^(.*?) (?:final )?int (light_image_picker_[^=]+)=.*/$1 final String $2="$2";/;
    print $out_handle $_;
  }
}
